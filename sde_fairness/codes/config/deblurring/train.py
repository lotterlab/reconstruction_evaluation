...
def load_classifier_models(config, device):
    if config["datasets"]["dataset"] == "chex":
        classifier = torch.load(
            config["classifier_path"], map_location=device, weights_only=False
        )
...
def main():
    ...
    train_loader = create_dataloader(
        train_set, opt, reweighting=opt["fairness_method"] == "reweighting"
    )
    logger.info(
    ...
    val_loader = create_dataloader(
        val_set, opt, reweighting=opt["fairness_method"] == "reweighting"
    )
    ...
    if opt["fairness_method"] == "eodd":
        fairness_loss = EODDLoss(
            classifier_models, fairness_lambda=opt["fairness_lambda"]
        )
    elif opt["fairness_method"] == "adversarial":
        fairness_loss = AdversarialLoss(
            classifier_models, fairness_lambda=opt["fairness_lambda"]
        )
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for train_data in train_bar:
            ...
            model.feed_data(
                states, LQ, GT, labels=labels, protected_attrs=protected_attrs
            )
        ...
        for idx, val_data in enumerate(val_loader):
            ...
            noisy_state = sde.noise_state(LQ)
            model.feed_data(
                noisy_state, LQ, GT, labels=labels, protected_attrs=protected_attrs
            )
            model.test(sde)
            visuals = model.get_current_visuals()
