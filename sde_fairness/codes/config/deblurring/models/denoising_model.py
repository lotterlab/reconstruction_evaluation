...
class DenoisingModel(BaseModel):
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        fairness_loss = self.fairness_loss(
            xt_1_optimum, self.labels, self.protected_attrs
        )

        # Update running averages
        self.running_recon_loss = (
            self.beta * self.running_recon_loss + (1 - self.beta) * recon_loss.item()
        )
        self.running_fair_loss = (
            self.beta * self.running_fair_loss + (1 - self.beta) * fairness_loss.item()
        )

        # Normalize losses if we have non-zero running averages
        if self.running_recon_loss > 0 and self.running_fair_loss > 0:
            normalized_fair_loss = fairness_loss * (
                self.running_recon_loss / self.running_fair_loss
            )
