...
class StereoLQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["data_type"] == "lmdb":
            ...
            resolution = None
        img_GT_L = util.read_img(
            self.GT_env, GT_path_L, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]
        img_GT_R = util.read_img(
            self.GT_env, GT_path_R, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]
        ...
        if self.opt["color"]:
            ...
            img_LR_L, img_LR_R = util.channel_convert(
                C, self.opt["color"], [img_LR_L, img_LR_R]
        ...
        img_GT_L = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_GT_L, (2, 0, 1)))
        ).float()
        img_GT_R = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_GT_R, (2, 0, 1)))
        ).float()
        img_LR_L = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_LR_L, (2, 0, 1)))
        ).float()
        img_LR_R = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_LR_R, (2, 0, 1)))
        ).float()
