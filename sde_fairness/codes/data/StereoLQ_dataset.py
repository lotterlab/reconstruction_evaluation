...
class StereoLQDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["data_type"] == "lmdb":
            ...
            resolution = None
        imgL_LR = util.read_img(
            self.LR_env, LR_path_L, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]
        imgR_LR = util.read_img(
            self.LR_env, LR_path_R, resolution
        )  # return: Numpy float32, HWC, BGR, [0,1]
        ...
        imgL_LR = torch.from_numpy(
            np.ascontiguousarray(np.transpose(imgL_LR, (2, 0, 1)))
        ).float()
        imgR_LR = torch.from_numpy(
            np.ascontiguousarray(np.transpose(imgR_LR, (2, 0, 1)))
        ).float()
