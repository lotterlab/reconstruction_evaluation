...
This repository is a fork of the original CycleGAN and pix2pix PyTorch implementation, specifically adapted for medical image reconstruction tasks. The codebase has been modified to support training on medical imaging datasets including UCSF and CheXpert datasets, with specialized data loaders and model configurations for medical image denoising and reconstruction.

### Training Configuration

To train on medical imaging datasets, use the following command structure. **Note**: Replace the example paths with your actual dataset and model paths.

```bash
# For UCSF dataset training:
python train.py \
  --dataroot /path/to/your/ucsf/dataset \
  ...
  --use_wandb \
  --val \
  [--model_g_path /path/to/pretrained/generator.pth] \
  [--classifier_path /path/to/classifier.pt]

# For CheXpert dataset training:
python train.py \
  --dataroot_A /path/to/noisy/images \
  --csv_path_A /path/to/noisy/metadata.csv \
  --dataroot_B /path/to/clean/images \
  --csv_path_B /path/to/clean/metadata.csv \
  --name your_chex_experiment_name \
  --model pix2pix \
  --dataset_mode chex \
  --direction AtoB \
  --input_nc 1 \
  ...
  --val \
  [--model_g_path /path/to/pretrained/generator.pth] \
  [--model_d_path /path/to/pretrained/discriminator.pth] \
  [--classifier_path /path/to/classifier.pt]
```

### Key Parameters

- `--dataset_mode`: Use `ucsf` or `chex` for the respective medical datasets
- `--model_g_path`: Optional path to a pretrained generator for continued training
- `--model_d_path`: Optional path to a pretrained discriminator for continued training
- `--classifier_path`: Optional path to an external classifier for evaluation
- `--lambda_L1`: L1 loss weight (typically set to 1000 for medical images)
- `--input_nc` and `--output_nc`: Set to 1 for grayscale medical images
- `--fairness_mode`: Defines which fairness method to run. Options include eodd, adversarial, and reweighting

For detailed installation instructions and additional parameters, please refer to the original documentation below.
