...
class Pix2PixModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.protected_attrs = input["protected_attrs"].to(self.device)
    ...
    def backward_G(self):
        ...
        raw_fairness_loss = self.fairness_loss(
            self.fake_B, self.labels, self.protected_attrs
        )

        # Update running averages
        self.running_l1_loss = (
            self.loss_momentum * self.running_l1_loss
            + (1 - self.loss_momentum) * raw_l1_loss.item()
        )
        self.running_fairness_loss = (
            self.loss_momentum * self.running_fairness_loss
            + (1 - self.loss_momentum) * raw_fairness_loss.item()
        )
