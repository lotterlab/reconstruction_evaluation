...
def init_net(net, init_type="normal", init_gain=0.02, gpu_ids=[]):
    ...
    if len(gpu_ids) > 0:
        ...
        net.to(0)
        net = torch.nn.DataParallel(net, [0])  # multi-GPUs
...
def define_D(
    ...
    if netD == "basic":  # default PatchGAN classifier
        ...
        net = PixelDiscriminator(
            input_nc, ndf, norm_layer=norm_layer, leaky_relu_alpha=leaky_relu_alpha
        )
    else:
        raise NotImplementedError(
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(
        self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, leaky_relu_alpha=0.2
    ):
