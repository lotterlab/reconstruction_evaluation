...
from fairness.eodd import EoddLoss
from fairness.adversarial import AdversarialLoss
...
def load_classifier_models(dataset_mode, tum, device):
    if dataset_mode == "chex":
        classifier_path = (
            "/vol/miltank/projects/practical_SoSe24/mri_inr/matteo/code/torchxrayvision/chex-densenet-class_norm01-best.pt"
            if tum
            else "/lotterlab/users/matteo/code/torchxrayvision/output/class_norm01/chex-densenet-class_norm01-best.pt"
        )
        ...
        ttype_path = (
            ""
            if tum
            else "/lotterlab/users/matteo/models/ttype/checkpoints/ttype_test_20240925_113544_epoch_1_best.pth"
        )
        tgrade_path = (
            ""
            if tum
            else "/lotterlab/users/matteo/models/tgrade/checkpoints/tgrade_test_20240925_113550_epoch_1_best.pth"
        )
...
if __name__ == "__main__":
    ...
    dataset = create_dataset(
        opt, reweight=opt.fairness_mode == "reweighting"
    ...
    if opt.val:
        val_dataset = create_dataset(
            opt, train=False, reweight=opt.fairness_mode == "reweighting"
        )
    ...
    if opt.model_d_path:
        state_dict = torch.load(
            opt.model_d_path, map_location=torch.device(model.device)
        )
        fixed_state_dict = {"module." + k: v for k, v in state_dict.items()}
    ...
    if opt.fairness_mode == "eodd":
        fairness_loss = EoddLoss(classifier_models, opt.fairness_lambda)
    elif opt.fairness_mode == "adversarial":
        fairness_loss = AdversarialLoss(classifier_models, opt.fairness_lambda)
    elif opt.fairness_mode == "reweighting":
        fairness_loss = IdentityLoss()
    else:
        raise ValueError(f"Invalid fairness mode: {opt.fairness_mode}")
