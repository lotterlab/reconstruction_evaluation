...
class Visualizer:
    ...
    def display_current_results(
        self, visuals, total_iters, epoch, save_result, train_mode=True
    ):
    ...
    def plot_current_losses(self, total_iters, losses, train_mode=True):
        ...
        try:
            self.vis.line(
                ...
                win=self.display_id if train_mode else (self.display_id + 100),
            )  # Different window IDs for train/val
        except VisdomExceptionBase:
            self.create_visdom_connections()
        ...
        if self.use_wandb:
            ...
            if train_mode:
                ...
                self.wandb_run.log(
                    prefixed_losses, step=total_iters
                )  # Use same step counter for consistency

    # losses: same format as |losses| of plot_current_losses
    def print_current_losses(
        self, epoch, iters, losses, t_comp, t_data, train_mode=True
    ):
        ...
        if not train_mode:
            ...
            message = ""
        message = message + "(epoch: %d, iters: %d, time: %.3f, data: %.3f) " % (
            epoch,
            iters,
            t_comp,
            t_data,
