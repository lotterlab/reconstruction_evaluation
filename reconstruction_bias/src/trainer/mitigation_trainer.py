...
class Trainer:
    ...
    def train(self):
        for epoch in range(1, self.num_epochs + 1):
            ...
            # Validation phase
            val_loss, val_metric, val_fairness_loss, val_l1_loss = self.validate_epoch(
                epoch
            )
            ...
            self.writer.add_scalars(
                "Loss",
                {
                    "Train": train_loss,
                    "Validation": val_loss,
                    "Val Fairness Loss": val_fairness_loss,
    ...
    def train_epoch(self, epoch):
        ...
        for x, y, protected_attrs, labels in train_bar:
            ...
            if self.criterion_ema is None:
                ...
                self.criterion_ema = (
                    self.ema_alpha * self.criterion_ema
                    + (1 - self.ema_alpha) * criterion_loss.item()
                )
                self.fairness_ema = (
                    self.ema_alpha * self.fairness_ema
                    + (1 - self.ema_alpha) * fairness_component.item()
                )
            ...
            if self.criterion_ema > 0 and self.fairness_ema > 0:
                normalized_fairness = fairness_component * (
                    self.criterion_ema / self.fairness_ema
                )
            else:
                normalized_fairness = fairness_component
    ...
    def validate_epoch(self, epoch):
        ...
        with torch.no_grad():
            for x, y, protected_attrs, labels in val_bar:
                ...
                fairness_component = self.fairness_loss(
                    outputs, labels, protected_attrs
                )
